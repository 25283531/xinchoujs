/**
 * 数据库迁移脚本：向 salary_items 表添加 subsidy_cycle 列
 */

import { Database } from '../database';
import { Migration } from './migration';

export class AddSubsidyCycleToSalaryItems implements Migration {
  readonly version = 2023060102; // 使用一个唯一的版本号，大于之前的迁移
  readonly name = 'Add subsidy_cycle to salary_items';
  ...
  async up(): Promise<void> {
    ...
    try {
      ...
      // 检查并添加 subsidy_cycle 列
      if (!tableInfo.some((column: any) => column.name === 'subsidy_cycle')) {
        try {
          console.log('添加 subsidy_cycle 列...');
          // SQLite ALTER TABLE ADD COLUMN 语法
          await db.exec(`ALTER TABLE salary_items ADD COLUMN subsidy_cycle VARCHAR;`);
          console.log('subsidy_cycle 列添加成功');
        } catch (error) {
          console.error('添加 subsidy_cycle 列失败:', error);
          throw error; // 添加列失败是严重错误，中断迁移
        }
      } else {
        console.log('subsidy_cycle 列已存在，跳过添加');
      }
  ...
  async down(): Promise<void> {
    ...
    try {
      ...
      // 为了简单起见，这里不实现回滚逻辑
      console.log('此迁移不支持自动回滚 subsidy_cycle 列的删除。');
    } catch (error) {
